...
1. 正确进入 U 态后，程序的特征还应有：使用 S 态特权指令，访问 S 态寄存器后会报错。请同学们可以自行测试这些内容（参考 `前三个测例 <https://github.com/LearningOS/uCore-Tutorial-Test/tree/main/src>`_ ，描述程序出错行为，同时注意注明你使用的 sbi 及其版本。

   .. note::

      部分资料中这三个程序以 ``__ch2_`` 为前缀，默认无法被测试触发。
      若你希望在本地通过 ``make test`` 运行这三个测试，请修改这三个测例的  ``__ch2_`` 前缀改为 ``ch2b_`` ，然后 ``make clean`` 再进行测试即可。
      详细测试方式请参考 ch0 中测例选择与编译选项部分章节。

2. 请结合用例理解 `trampoline.S <https://github.com/LearningOS/uCore-Tutorial-Code/blob/ch3/os/trampoline.S>`_ 中两个函数 `userret` 和 `uservec` 的作用，并回答如下几个问题:
